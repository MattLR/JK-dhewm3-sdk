...
/*
================
jkVehiclePosition::jkVehiclePosition
================
*/
jkVehiclePosition::jkVehiclePosition ( void ) {	
...
/*
================
jkVehiclePosition::Init
================
*/
void jkVehiclePosition::Init ( jkVehicle* parent, const idDict& args ) {
	idVec3 garbage;
...
// jmarshall - vehicle sound loops cause a crash, disabling for the time being.
	...
	//	mSoundPart = AddPart ( jkVehicleSound::GetClassType(), args );
	//	static_cast<jkVehicleSound*>(mParts[mSoundPart])->SetAutoActivate ( false );
	//}
// jmarshall end
...
/*
================
jkVehiclePosition::SetParts
================
*/
void jkVehiclePosition::SetParts ( const idDict& args ) {
	const idKeyValue* kv;
	...
	while ( kv ) {
		...
		if ( !typeInfo || !typeInfo->IsType ( jkVehiclePart::Type ) ) {
...
/*
================
jkVehiclePosition::AddPart
================
*/
int jkVehiclePosition::AddPart ( const idTypeInfo &classdef, const idDict& args ) {
	jkVehiclePart*	part;
	int				soundChannel;
	...
	part = static_cast<jkVehiclePart*>(classdef.CreateInstance ( ));
	...
	if ( part->IsType ( jkVehicleWeapon::Type ) ) {
...
/*
================
jkVehiclePosition::SetDriver
================
*/
bool jkVehiclePosition::SetDriver ( idActor* driver ) {
	if ( mDriver == driver ) {
		return false;
	}
	...
	if ( driver ) {
		...
		if ( mSoundPart != -1 ) {
			static_cast<jkVehicleSound*>(mParts[mSoundPart])->Play ( );
		}
		
		// Bind the driver to a joint or to the vehicles origin?
		//DYNAMIX QUAKE WARS for reference
		/*
				================
		...
		*/
		...
		if ( mSoundPart != -1 ) {
			static_cast<jkVehicleSound*>(mParts[mSoundPart])->Stop ( );
		}
...
/*
================
jkVehiclePosition::GetAxis
================
*/
idMat3 jkVehiclePosition::GetAxis() const {
	return mAxisOffset * GetParent()->GetPhysics()->GetAxis();
}

/*
================
jkVehiclePosition::GetOrigin
================
*/
idVec3 jkVehiclePosition::GetOrigin( const idVec3& offset ) const {
	return GetParent()->GetPhysics()->GetOrigin() + (mDriverOffset + offset) * GetAxis();
}

/*
================
jkVehiclePosition::ActivateParts
================
*/
void jkVehiclePosition::ActivateParts ( bool active ) {
	int i;
...
/*
================
jkVehiclePosition::EjectDriver
================
*/
bool jkVehiclePosition::EjectDriver ( bool force ) {
	if ( !IsOccupied ( ) ) {
		return false;
	}
...
/*
================
jkVehiclePosition::SetInput
================
*/
void jkVehiclePosition::SetInput ( const usercmd_t& cmd, const idAngles& newAngles ) {
	/*
...
/*
================
jkVehiclePosition::GetInput
================
*/
void jkVehiclePosition::GetInput( usercmd_t& cmd, idAngles& newAngles ) const {
	cmd = mInputCmd;
...
/*
================
jkVehiclePosition::UpdateHUD
================
*/
void jkVehiclePosition::UpdateHUD ( idUserInterface* gui ) {
	...
	if ( mCurrentWeapon >= 0 && mCurrentWeapon < mWeapons.Num() ) {
		jkVehicleWeapon* weapon;
		weapon = static_cast<jkVehicleWeapon*>(mWeapons[mCurrentWeapon]);
...
/*
================
jkVehiclePosition::UpdateCursorGUI
================
*/
void jkVehiclePosition::UpdateCursorGUI ( idUserInterface* gui ) {
	if ( mCurrentWeapon < 0 || mCurrentWeapon >= mWeapons.Num() ) {
		return;
	}
...
/*
================
jkVehiclePosition::UpdateInternalView
================
*/
void jkVehiclePosition::UpdateInternalView ( bool force ) {
	bool internal = false;
...
/*
================
jkVehiclePosition::RunPrePhysics
================
*/
void jkVehiclePosition::RunPrePhysics ( void ) {
	int i;
	...
	if ( mSoundPart != -1 && mSoundMaxSpeed > 0.0f ) {
		...
		static_cast<jkVehicleSound*>(mParts[mSoundPart])->Attenuate ( f, f );
	}
...
/*
================
jkVehiclePosition::RunPostPhysics
================
*/
void jkVehiclePosition::RunPostPhysics ( void ) {
	int i;
	...
	if ( fl.inputValid ) {
		...
		//if ( (mInputCmd.flags & UCF_IMPULSE_SEQUENCE) != (mOldInputFlags & UCF_IMPULSE_SEQUENCE) ) {
			//if ( mInputCmd.impulse == IMPULSE_40  ) {
			gameLocal.DPrintf("Exit vehicle key pressed\n");
			
			// inform the driver that its time to get out of the vehicle.
			//if( !mDriver->EventIsPosted(&AI_ExitVehicle) ) {
				mDriver->PostEventMS( &AI_ExitVehicle, 250, false );// To remove jump when exiting FIXME1 Dynamix
			//}
...
/*
================
jkVehiclePosition::GetEyePosition
================
*/
void jkVehiclePosition::GetEyePosition( idVec3& origin, idMat3& axis ) const {
...
/*
================
jkVehiclePosition::GetDriverPosition
================
*/
void jkVehiclePosition::GetDriverPosition( idVec3& origin, idMat3& axis ) const {
	if( fl.bindDriver ) {
...
/*
================
jkVehiclePosition::FireWeapon
================
*/
void jkVehiclePosition::FireWeapon( void ) {
	if ( mCurrentWeapon >= 0 ) {
		static_cast<jkVehicleWeapon*>( mWeapons[mCurrentWeapon] )->Fire();
	}
}

/*
================
jkVehiclePosition::SelectWeapon
================
*/
void jkVehiclePosition::SelectWeapon ( int weapon ) {
...
// mekberg: clear effect
	if ( mCurrentWeapon != -1 ) {
		static_cast<jkVehicleWeapon*> ( mWeapons[ mCurrentWeapon ] )->StopTargetEffect( );
	}
...
/*
================
jkVehiclePosition::WriteToSnapshot
================
*/
void jkVehiclePosition::WriteToSnapshot ( idBitMsgDelta &msg ) const {
	msg.WriteBits ( mCurrentWeapon, 3 );
}

/*
================
jkVehiclePosition::ReadFromSnapshot
================
*/
void jkVehiclePosition::ReadFromSnapshot ( const idBitMsgDelta &msg ) {
	mCurrentWeapon = msg.ReadBits ( 3 );
}

/*
================
jkVehiclePosition::Save
================
*/
void jkVehiclePosition::Save ( idSaveGame* savefile ) const {
	int i;
...
/*
================
jkVehiclePosition::Restore
================
*/
void jkVehiclePosition::Restore ( idRestoreGame* savefile ) {
	int i;
	int num;
	...
	for ( i = 0; i < num; i ++ ) {
		...
		part = static_cast<jkVehiclePart*>(typeInfo->CreateInstance ( ));
